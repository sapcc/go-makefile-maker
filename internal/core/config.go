...
func getDefaultBranch() string {
	// works when cloned but only with one remote
	b, err := exec.Command("git", "symbolic-ref", "refs/remotes/origin/HEAD").CombinedOutput()
	if err != nil {
		logg.Error("git symbolic-ref: %s", err.Error())
	} else {
	...
	// works in GitHub Actions and with multiple remotes
	logg.Info("Trying next `git ls-remote`")
	b, err = exec.Command("git", "ls-remote", "--symref", "origin", "HEAD").CombinedOutput()
	if err != nil {
		logg.Error("git ls-remote: %s", err.Error())
	}
	// branch := strings.TrimPrefix(string(b), "refs/heads/")
...
func (c *Configuration) Validate() error {
	...
	if ghwCfg != nil {
		...
		if ghwCfg.Global.DefaultBranch == "" {
			...
			if branch == "" {
				return fmt.Errorf("could not find default branch using git, you can define manually be setting 'githubWorkflow.global.defaultBranch' in config")
			}
			c.GitHubWorkflow.Global.DefaultBranch = branch
		}
