	...
	if isSAPCC {
		...
		dev.addRule(rule{
			...
			recipe: []string{
				...
				fmt.Sprintf(`@printf "%%s\0" %s | xargs -0 -I{} bash -c 'year="$$(grep 'Copyright' {} | head -n1 | grep -E -o '[0-9]{4}')"; gawk -i inplace '"'"'{if (display) {print} else {!/^\/\*/ && !/^\*/}}; {if (!display) {/^(package |$$)/{print;display=1}} else { }}'"'"' {}; addlicense -c "SAP SE" -s=only -y "$$year" %s {}; sed -i '"'"'1s+// Copyright +// SPDX-FileCopyrightText: +'"'"' {}'`, allSourceFilesExpr, ignoreOptionsStr),
