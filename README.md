...
You can also define [`customManagers`](https://docs.renovatebot.com/modules/manager/regex/). Note that only the fields mentioned above are accepted when defining a `customManagers`. An example to detect `ENVTEST_K8S_VERSION` env variable version and update it in `Makefile`
